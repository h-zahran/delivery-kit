...
if [ "$ctx" -gt "$WINDOW" ]; then
  ...
  if [ ! -f "$wflag" ]; then
    : 2>/dev/null > "$wflag"
    misconfig=" WINDOW MISCONFIGURED: observed context (${ctx} tokens) exceeds the configured window (${WINDOW} tokens), so contextGuard.windowTokens in .delivery-kit.json is too small for this model. Raise it and the percentages above become meaningful."
  fi
fi
