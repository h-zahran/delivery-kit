...
- [X] T018 [US1] Review `git diff` line by line against T003's inventory: 43 changed lines across 17 files (37 from the inventory, plus 6 drive-rooted sites found at review in spellings the four shapes never covered), and every one bar FR-006's reworded pointer differs by a path token alone (FR-001, FR-004, FR-005, FR-007, SC-002). Confirm every test-runner replacement is byte-identical to the spelling `CONTRIBUTING.md:9` already documents (FR-002), and that `specs/006-release-1-1-0/quickstart.md:232` still resolves `$BATS` then `PATH` before its fallback (FR-003).
