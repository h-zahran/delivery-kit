...
- [X] T003 [P] Confirm an external `timeout` program is on the path with `command -v timeout`; record the result in `.delivery-kit/runs/008-progress-coverage-timeout/baseline-inventory.txt`. **Without it the per-test limit silently does nothing**, so this is a prerequisite and not a nicety
...
- [X] T027 [US3] Append the C7 test to `pipeline/tests/progress.bats`: fewer than two arguments produces the usage refusal; assert several subcommands including the first and the last, so the enumeration cannot rot silently
