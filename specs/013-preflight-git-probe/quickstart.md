...
```bash
bash pipeline/scripts/preflight.sh | jq '.capabilities'
```

Expected: an object with four members, and `"git": true`.
...
awk '/^The script only reports; the decisions are yours/,/^\*\*Base branch:\*\*/' \
  pipeline/skills/pipeline/SKILL.md | grep -n '^11\.' -A 12
```

Expected: decision item 11 exists, says it fires before every other decision,
names `https://git-scm.com/downloads`, and says git is never a `willSkip`
entry.
...
Expected: green. This suite slices the probe block and the decision walk and
greps pinned sentences inside each; a reworded line there turns it red.
